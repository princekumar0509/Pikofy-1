...
// ALL debts (1-to-1 AND group) netted against cases where the user
...
export const getUsersWithOutstandingDebts = query({
  handler: async (ctx) => {
    ...
    // Load ALL expenses (including group expenses)
    const expenses = await ctx.db.query("expenses").collect();

    // Load ALL settlements (including group settlements)
